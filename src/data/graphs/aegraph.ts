...
export const aegraphGraph = () => {
  ...
  graph.createEdge("lumina", "aegraph", { type: "subcomponent of" });

  graph.createNode({ id: "reactflow", type: "graph software" });
  graph.createNode({ id: "graphiz", type: "graph software" });
  graph.createEdge("aegraph", "reactflow", { type: "adaptor for" });
  graph.createEdge("aegraph", "graphiz", { type: "adaptor for" });
